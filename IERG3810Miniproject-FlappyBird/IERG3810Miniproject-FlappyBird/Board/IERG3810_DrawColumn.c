...
void clear_column_part(int old_column_x, int column_move_speed, int bottom_level, int top_level){
	 ...
	 int top_y = level_to_y(top_level,1) - 10 -4;
	 int bottom_y = level_to_y(bottom_level,0)+ 10;
		 
	 int length_x = (temp_old_x + 37) - (new_x + 32);
	 ...
	 if(length_x > 0){
		 IERG3810_TFTLCD_FillRectangle(screen_color,start_x, length_x+3, top_y, length_y_top);
		 IERG3810_TFTLCD_FillRectangle(screen_color,start_x, length_x+3, 0, length_y_bottom+3);
