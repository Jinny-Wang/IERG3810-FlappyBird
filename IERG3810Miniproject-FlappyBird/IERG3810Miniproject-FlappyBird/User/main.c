...
u16 screen_color = 0xAFFF; //0xFE7F pink; //0xAFFF lightlue; //0x0000 : black.
...
void bird_fall(int step){
	...
	if(task1HeartBeat > 1){
				IERG3810_TFTLCD_FillRectangle(screen_color,bird_x, 34, bird_y, 28);						
				bird_y -= step; 				
...
void Initial_ui(u16 bgcolor,int timeout){
	...
	char sid1[10] = "1155076727";
		...
		//Show names and SIDs
		IERG3810_TFTLCD_ShowCharInLine(0x0050,0x0008,sid1,0x000A,0x0000,bgcolor);
		IERG3810_TFTLCD_ShowCharInLine(0x0050,0x0020,sid2,0x000A,0x0000,bgcolor); 
		
		IERG3810_TFTLCD_ShowChinChar(0x20,0x08,0x00,0x0000,bgcolor);//Wang
		IERG3810_TFTLCD_ShowChinChar(0x30,0x08,0x02,0x0000,bgcolor);//Jing
		IERG3810_TFTLCD_ShowChinChar(0x40,0x08,0x03,0x0000,bgcolor);//Yu
		
		IERG3810_TFTLCD_ShowChinChar(0x20,0x20,0x01,0x0000,bgcolor); //Xie
		IERG3810_TFTLCD_ShowChinChar(0x30,0x20,0x04,0x0000,bgcolor); //Yu
		IERG3810_TFTLCD_ShowChinChar(0x40,0x20,0x05,0x0000,bgcolor); //Xiao
...
void print_score(void){
	int i;
	char score_display[17] = "Your Score:     \n";
	int score_length = num_of_digit(score);
	int temp_score = score;
	  //score_display[15] = score_length + '0';
	for(i = 0; i < score_length; i++){
	 ...
	 USART_print(2,score_display);
}

void print_column_x(void){
	
	int i,j,k;
	char column1_x_print [17] = "Column1_x:      \n";
	char column2_x_print [17] = "Column2_x:      \n";
	char column3_x_print [17 ] = "Column3_x:      \n";
	
	int x1_length = num_of_digit(column1_x);
	int x2_length = num_of_digit(column2_x);
	int x3_length = num_of_digit(column3_x);
	int temp_x1 = column1_x;
	int temp_x2 = column2_x;
	int temp_x3 = column3_x;
	
	  //score_display[15] = score_length + '0';
	for(i = 0; i < x1_length; i++){
		char digit = temp_x1%10 + '0';
		column1_x_print[14-i] = digit;
		 temp_x1  /= 10;	
	 }
	 USART_print(2,column1_x_print);
	
	for(j = 0; j < x2_length; j++){
		char digit = temp_x2%10 + '0';
		column2_x_print[14-j] = digit;
		 temp_x2  /= 10;	
	 }
	 USART_print(2,column2_x_print);
	 
	 for(k = 0; k < x1_length; k++){
		char digit = temp_x3%10 + '0';
		column3_x_print[14-k] = digit;
		 temp_x3  /= 10;	
	 }
	 USART_print(2,column3_x_print);
}

void column_move(int column_move_speed, int score_count_speed , int column_refresh_speed){
	
	
	if(column1_x+score_count_speed== bird_x){
		score++;
		//print_column_x();
		print_score();
	}		
	if(column2_x+score_count_speed== bird_x){
		score++;
		//print_column_x();
		print_score();
	}	
	if(column3_x+score_count_speed== bird_x){
		score++;
		//print_column_x();
		print_score();
	}			
	...
	if(column1_x + column_refresh_speed < 0){
		column1_x = 240;
		levels[0] =  rand()%9;
		levels[1] =  rand()%9;
	}
	if(column2_x+column_refresh_speed < 0) {
		column2_x = column1_x+100;
		levels[2] =  rand()%9;
		levels[3] =  rand()%9;
	}
	if(column3_x+column_refresh_speed < 0){
		column3_x = column1_x+200;	
		levels[4] =  rand()%9;
		levels[5] =  rand()%9;
	}
...
int main(void){
	  ...
	  int bird_fall_speed,column_move_speed, bird_fly_speed, score_count_speed , column_refresh_speed;
		int i,m,n;  
		int timeout = 20000;
    ...
    IERG3810_PS2key_ExtiInit();
	  ...
	  //IERG3810_TFTLCD_FillRectangle(u16 color,u16 start_x, u16 length_x, u16 start_y, u16 length_y);
		...
		while(1){
			...
			switch(difficulty_level){
			 case(1):
				 ...
				 column_move_speed = 1;
				 bird_fly_speed = 20;
			   score_count_speed = 38;
			   column_refresh_speed = 44;
				 break;
			 case(2):
				 bird_fall_speed = 2;
				 column_move_speed = 2;
			...
			IERG3810_TFTLCD_FillRectangle(screen_color,0, 240, 0, 320);
			//The game body
        ...
        keyboard_control_bird(timeout,bird_fly_speed);
          ...
          column_move(column_move_speed,score_count_speed,column_refresh_speed);
			    task2HeartBeat = 0;
					
			  }		  
    
			}
			game_over(0xFFFA); //gameover_background : light yellow
